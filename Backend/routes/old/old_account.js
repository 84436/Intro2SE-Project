...
app.get("/", async (i, o) => {
    try {
        if (i.body.hasOwnProperty("token") == false) {
            let response = await accountModel.find({}, { password: 0 }, (err) => {
            ...
            if (doc && Object.keys(doc).length > 0) {
                await accountModel.findOne({ "email": doc.email }, (err, acc) => {
                    if (err) throw new Error("Something went wrong with find function.");
...
app.delete("/", async (i, o) => {
    ...
    try {
        ...
        if (doc && Object.keys(doc).length > 0) {
            // find and remove
            let acc = await accountModel.findOne({ "email": doc.email }, (err) => {
                if (err) throw new Error("Something went in find function.");
            })
            if (!(acc && Object.keys(acc).length > 0))
                return o.status(404).send({ "error": "Cannot find email from token" })
            else {
                await accountModel.findOneAndRemove({ "email": doc.email }, (err) => {
    ...
    try {
        ...
        if (doc && Object.keys(doc).length > 0) {
            ...
            if (i.body.hasOwnProperty("email")) {
                ...
                let doc4 = await accountModel.findOne({ "email": i.body.email }, (err) => {
                    if (err) throw new Error("Something went in find function");
                })
            ...
            // update
            await accountModel.findOneAndUpdate({ "email": doc.email }, acc, (err) => {
                if (err) throw new Error("Something went in update function");
            })
            if (i.body.hasOwnProperty("email")) {
...
app.post("/login", async (i, o) => {
    try {
        ...
        acc = await accountModel.findOne(account, (err) => { // Verify password, the lazy way
            if (err) throw new Error("Something went in find function");
        })
        if (acc && Object.keys(acc).length > 0) {
